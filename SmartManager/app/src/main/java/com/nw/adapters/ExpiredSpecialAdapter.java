...
public class ExpiredSpecialAdapter extends ArrayAdapter<SpecialVehicle>{

	ImageLoader imageLoader;
	TaskListener mListener,endListener;
	DisplayImageOptions options;
	
	public ExpiredSpecialAdapter(Context context, int resource, List<SpecialVehicle> objects, TaskListener listener,TaskListener endListener) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if(convertView==null)
		{
			convertView=LayoutInflater.from(getContext()).inflate(R.layout.list_item_expired_specials, parent,false);
		...
		if(getItem(position).getUsedYear()==0){
			holder.tvTitle.setText(Html.fromHtml("<font color=#ffffff>Year?</font> "
					+ "<font color="+getContext().getResources().getColor(R.color.dark_blue)+">"+
		...
		holder.btnEnd.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				CustomDialogManager.showOkCancelDialog(getContext(), getContext().getString(R.string.are_you_sure_you_want_delete_this_special), new DialogListener() {
	...
	private static class ViewHolder{
		ImageView ivVehicle;
		TextView tvTitle, tvDescription, tvColor, tvStockNo, tvMileage, tvExpired, tvStatus;
		...
		public void init(View convertView){
			...
			tvExpired= (TextView) convertView.findViewById(R.id.tvExpired);
	...
	private void endSpecial(final int position)
	{
		...
		if(HelperHttp.isNetworkAvailable(getContext()))
		{
			...
			parameterList.add(new Parameter("isExpired", true, Boolean.class));
